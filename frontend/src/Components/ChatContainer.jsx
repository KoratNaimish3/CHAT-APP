...
function ChatContainer() {
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 mx-4 border-b border-stone-500 py-2'>
      ...
      <div className='flex flex-col gap-2 overflow-y-scroll p-4 pb-6 h-[calc(100%-110px)]'>
        {messages.map((msg, index) => (
          ...
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
            ...
            {msg.image ? (
              ...
              <p className={`p-2 max-w-[200px] md:text-sm mb-8 font-light rounded-lg break-all bg-violet-500/30 text-white ${msg.senderId === authUser._id ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>
            )}
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        ...
        <div className=' flex-1 flex items-center bg-gray-100/12 px-3 rounded-full gap-2'>
          ...
          <input type="file" id='image' accept='image/png , image/jpeg , image/jpg' hidden onChange={handleSendImage} />
